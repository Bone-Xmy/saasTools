...
//import org.saas.qa.domain.Document;
import org.saas.qa.domain.Document;
	...
	@RequestMapping(value="/document/addDocument")
	 public ModelAndView addDocument(
		...
		if(flag.equals("1")){
			...
			// 上传文件路径
			String path = myDir.getDir();
			System.out.println(path);
			for(int i = 0; i < document.getFile().length; i++){
				// 上传文件名
				String fileName = document.getFile()[i].getOriginalFilename();
			 	// 将上传文件保存到一个目标文件当中
				document.getFile()[i].transferTo(new File(path+File.separator+ fileName));
			}
